...
class PricingCalculator {
    constructor() {
        this.basePrices = {
            ...
            defaultMusic: 0,        // Free default background music
            customMusic: 5000,      // Custom background music +5k
            bookEnabled: 20000,     // Book enabled +20k
            extraPage: 3000,        // Each extra page (>10 pages) +3k
            heartEffect: 5000,      // Heart effect +5k
            saveForever: 20000      // Save QR permanently +20k
        };
        ...
        this.initializeVoucherSystem(); // ✅ Add this line
        this.updatePricing();
    }

    createPricingUI() {
        // Tạo container cho pricing
        // Create container for pricing
...
`;
        ...
        if (tipInput) {
            tipInput.addEventListener('input', () => {
                ...
                // Save tip to settings (global or window.settings)
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            if (isCollapsed) {
                ...
                // Only show voucher section if user is logged in
                if (localStorage.getItem('user_uid')) {
                    voucherSection.style.display = 'block';
                }
                // Hiển thị payment method section nếu có giá tiền
                // Show payment method section if there is a price
    ...
    async loadUserVouchers() {
        ...
        voucherList.innerHTML = 'Loading vouchers...';
...
`).join('');
            ...
            voucherList.innerHTML = '<span style="color:#e53935;">Failed to load vouchers!</span>';
        }
    }
    ...
    handleVoucherChange(e) {
        ...
        if (e.target.checked) {
            ...
            // Deselect all, only select the clicked one
            ...
            // If clicking the selected one, deselect all
            this.selectedVoucher = null;
            if (voucherResult) {
    ...
    calculatePrice(currentSettings) {
        ...
        if (currentSettings.enableBook) {
            ...
            if (totalPages > 10) {
                ...
                items.push({
                    name: `Extra pages (${extraPages} pages)`,
            ...
            // 3. Check heart effect (only when book is present)
            if (currentSettings.enableHeart) {
                items.push({
                    name: 'Heart effect (in book)',
        ...
        // 4. Check save QR permanently
        if (currentSettings.isSave) {
            items.push({
                name: 'Save QR permanently',
    ...
    updatePricing(currentSettings = null) {
        ...
        // Use global settings if any, or default settings
    ...
    `).join('');
        ...
        if (finalPrice > 0) {
            ...
            totalPrice.style.color = '#ff9800'; // orange color
    ...
    `;
            ...
            if (paymentMethodSection) {
                ...
                // Setup payment method selection again when section is displayed (only if needed)
    ...
    `;
            ...
            if (paymentMethodSection) {
                ...
                this.paymentMethodSetup = false; // Reset flag when hidden
            }
        }

        // Lưu trữ giá hiện tại cho action button
        // Store current price for action button
    ...
    getSelectedPaymentMethod() {
        ...
        return selectedMethod ? selectedMethod.value : 'PAYOS'; // Default is PAYOS
    }

    // Xử lý khi click nút action - Luôn tạo free
    // Handle action button click - Always create free
    async handleActionButton() {
        ...
        try {
            ...
            // ✅ Validate total amount before doing anything else
            const selectedPaymentMethod = this.getSelectedPaymentMethod();
            if (this.currentTotal > 0 && this.currentTotal < 10000 && selectedPaymentMethod !== 'PAYPAL') {
                ...
                this.showNotification('❌ Minimum payment amount is 10,000 VND!', 'error');
                return; // Do not proceed
            }

            // ✅ Phân biệt Free vs Paid dựa trên giá tiền
            // ✅ Distinguish Free vs Paid based on price
            if (this.currentTotal > 0) {
                // Có giá tiền - xử lý thanh toán
                // Has price - process payment
                actionButton.textContent = '⏳ Processing payment...';
                await this.handlePaidCreation();
            } else {
                // Miễn phí - tạo trực tiếp
                // Free - create directly
        ...
        try {
            ...
            if (settingsToUse.enableBook) {
                ...
                if (totalPages > 1 && totalPages % 2 === 0) {
                    this.showNotification(`❌ Invalid page structure! Currently there are ${totalPages} pages. Please add or remove 1 page to create a valid structure.`, 'error');
                    return;
                }
            }
            ...
            if (settingsToUse.enableBook && settingsToUse.pages?.length > 0) {
                progressNotification = this.showProgressNotification('📤 Uploading book page images...');
            }
            ...
            if (progressNotification) {
                this.updateProgressNotification(progressNotification, '🌐 Creating website...');
            }
            ...
            if (!websiteResult.success) {
                throw new Error(websiteResult.error || 'Cannot create website');
            }

            // Tạo sản phẩm
            // Create product
            if (progressNotification) {
                this.updateProgressNotification(progressNotification, '📦 Creating product...');
            }
            ...
            // Process voucher if any (for FREE too)
            if (this.selectedVoucher) {
                if (progressNotification) {
                    this.updateProgressNotification(progressNotification, '🎫 Applying voucher...');
                }
        ...
        try {
            ...
            if (this.selectedVoucher) {
                ...
                try {
                    ...
                    finalPrice = this.currentTotal; // Price after applying voucher
                } catch (error) {
                    console.error('Lỗi áp dụng voucher:', error);
                    this.showNotification('⚠️ Error applying voucher, continuing payment with original price', 'warning');
                }
            }
            ...
            if (finalPrice > 0) {
                ...
                // Case where voucher makes price = 0
                this.showSuccessResult(websiteResult.websiteId, 0);
                this.finishCreation();
            }
    ...
    async applyVoucher(orderCode) {
        try {
            ...
            if (!voucherData.success) {
                throw new Error(voucherData.message || 'Failed to apply voucher!');
            }
    ...
    async processPayment(orderCode, amount, websiteId, paymentMethod = 'PAYOS') {
        try {
            ...
            // Save orderCode to localStorage for WebSocket to use
            localStorage.setItem('current_order_code', orderCode);

            const paymentMethodText = paymentMethod === 'PAYPAL' ? 'PAYPAL' : 'PAYOS';
            this.showNotification(`🔄 Redirecting to ${paymentMethodText} payment page...`, 'info');
            ...
            if (paymentMethod === 'PAYPAL') {
                ...
                // PayPal returns data.checkoutUrl
    ...
    showPaymentModal(checkoutUrl, websiteId, amount) {
        ...
        // Ensure close button and iframe always exist
        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (!wsMessageReceived) {
                    window.cleanupPaymentState(orderCode, false);
                    this.showNotification('Timeout - Cannot determine payment status!', 'warning');
                    reject(new Error('Timeout - Cannot determine payment status!'));
                }
            }, 300000); // 5 phút
        });
    }
    ...
    showPayPalPayment(checkoutUrl, websiteId, amount) {
        ...
        this.showNotification('🔄 Redirecting to PayPal payment page...', 'info');
    ...
    showSuccessResult(websiteId, price) {
        ...
        this.showNotification('🎉 Website created successfully!', 'success');
...
`;
        ...
        // After assigning innerHTML, add event to QR button
    ...
    getCurrentSettingsFromModal() {
        ...
        if (!settingsModal || settingsModal.style.display === 'none') {
            ...
            // Even if modal is closed, still get values from form if possible
        }
    ...
    // Add function to get pages from modal
    getPagesFromModal() {
        try {
            const pages = [];
            ...
            // Get number of pages from current settings
            ...
            currentPages.forEach((page, index) => {
                ...
                // Get content (only for pages with textarea)
                if (contentInput) {
    ...
    copyShareURL() {
        ...
        if (shareURL) {
            ...
            try {
                ...
                this.showNotification('❌ Cannot copy link!', 'error');
            }
        }
    }
    ...
    // Method to update price from outside
...
// Initialize calculator after DOM ready
...
// Function to initialize pricing calculator
